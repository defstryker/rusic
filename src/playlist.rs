...
impl Playlist {
    ...
    fn create_columns(treeview: &TreeView) {
        Self::add_pixbux_column(treeview, THUMBNAIL_COLUMN as i32, Visible);
        ...
        Self::add_pixbux_column(treeview, PIXBUF_COLUMN as i32, Invisible);
    }
    ...
    fn add_pixbux_column(treeview: &TreeView, column: i32, visibility: Visibility) {
        let view_column = TreeViewColumn::new();
    ...
    pub fn add(&self, path: &Path) {
        ...
        if let Ok(tag) = Tag::read_from_path(path) {
            ...
            self.model
                .set_value(&row, TRACK_COLUMN, &track_value.to_value()); // track or track_value ???
        } else {
            self.model
